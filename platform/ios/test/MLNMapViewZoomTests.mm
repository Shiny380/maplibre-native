...
@interface MLNMapViewZoomTests : XCTestCase
@property (nonatomic) MLNMapView *mapView;
@end

@implementation MLNMapViewZoomTests
...
- (void)testZoom {
  ...
  for (NSNumber *zoom in @[ @1, @5, @10, @15, @22 ]) {
    ...
    XCTAssertEqualWithAccuracy(
        originalCenterCoordinate.latitude, self.mapView.centerCoordinate.latitude, 0.0000001,
        "@Map center coordinate latitude should remain constant during zoom to z%@.", zoom);
    XCTAssertEqualWithAccuracy(
        originalCenterCoordinate.longitude, self.mapView.centerCoordinate.longitude, 0.0000001,
        @"Map center coordinate longitude should remain constant during zoom to z%@.", zoom);
  }
}
...
- (void)testZoomEnabled {
  ...
  // Disabled
  {
    ...
    [self.mapView handlePinchGesture:gesture];
    XCTAssertNotEqual(self.mapView.zoomLevel, log2(gesture.scale),
                      @"Gestural zoom should not work when zoom is disabled.");

    self.mapView.zoomLevel = 15.f;
    XCTAssertEqualWithAccuracy(self.mapView.zoomLevel, 15, 0.001,
                               @"Programmatic zoom is allowed when zoomEnabled = NO.");
  }

  // Enabled
  {
    // No need to reset the map zoom or gesture scale, since gesture scale hasn't been applied yet
    // and the map zoom will be overriden when the gesture is handled.
    self.mapView.zoomEnabled = YES;
    XCTAssertEqual(self.mapView.allowsZooming, YES);

    [self.mapView handlePinchGesture:gesture];
    XCTAssertEqualWithAccuracy(self.mapView.zoomLevel, log2(gesture.scale), 0.001,
                               @"Gestural zoom should work when zoom is enabled.");
  }
}
...
- (void)testPinchGesture {
  ...
  UIPinchGestureRecognizerMock *gesture =
      [[UIPinchGestureRecognizerMock alloc] initWithTarget:self.mapView action:nil];
  gesture.state = UIGestureRecognizerStateBegan;
  gesture.scale = 0;
  ...
  for (NSNumber *zoom in @[ @1, @5, @10, @15, @22 ]) {
    gesture.state = UIGestureRecognizerStateChanged;
    gesture.scale = MLNScaleFromZoomLevel([zoom doubleValue]);
    [self.mapView handlePinchGesture:gesture];
    XCTAssertEqual(self.mapView.zoomLevel, [zoom doubleValue],
                   @"Map zoom should match gesture to z%@.", zoom);

    // Given a hypothetical zoom into the exact center of the map, the center coordinate should
    // remain the same.
    XCTAssertEqualWithAccuracy(
        originalCenterCoordinate.latitude, self.mapView.centerCoordinate.latitude, 0.0000001,
        "@Map center coordinate latitude should remain constant during zoom to z%@.", zoom);
...
- (void)testPinchGestureOffCenter {
  ...
  // Set a map rotation so that we can later check if un-rotating happens around the correct center
  // point.
  self.mapView.direction = 45;

  // Zoom to z18 at the off-center origin.
  gesture.state = UIGestureRecognizerStateChanged;
  gesture.scale = MLNScaleFromZoomLevel(3);
  [self.mapView handlePinchGesture:gesture];
  XCTAssertEqual(self.mapView.zoomLevel, 18,
                 @"Map zoom should match original zoom plus gesture zoom.");
  ...
  CLLocationCoordinate2D manuallyDerivedCenterCoordinate =
      [self.mapView convertPoint:self.mapView.center toCoordinateFromView:nil];
  XCTAssertEqualWithAccuracy(centerCoordinateBeforeReset.latitude,
                             manuallyDerivedCenterCoordinate.latitude, 0.0000001,
                             "@Map center latitude should be equal to manually derived latitude.");
  XCTAssertEqualWithAccuracy(
      centerCoordinateBeforeReset.longitude, manuallyDerivedCenterCoordinate.longitude, 0.0000001,
      @"Map center longitude should be equal to manually derived longitude.");

  self.mapView.direction = 0;
  XCTAssertEqualWithAccuracy(
      centerCoordinateBeforeReset.latitude, self.mapView.centerCoordinate.latitude, 0.0000001,
      "@Map center coordinate latitude should remain constant after resetting to north.");
  XCTAssertEqualWithAccuracy(
      centerCoordinateBeforeReset.longitude, self.mapView.centerCoordinate.longitude, 0.0000001,
      @"Map center coordinate longitude should remain constant after resetting to north.");
}

- (void)testPinchAndZoom {
  [[NSUserDefaults standardUserDefaults] setObject:@3 forKey:@"MLNRotationThresholdWhileZooming"];
  self.mapView.rotationThresholdWhileZooming = 3;
  self.mapView.zoomLevel = 15;
  UIPinchGestureRecognizerMock *pinch =
      [[UIPinchGestureRecognizerMock alloc] initWithTarget:self.mapView action:nil];
  ...
  UIRotationGestureRecognizerMock *rotate =
      [[UIRotationGestureRecognizerMock alloc] initWithTarget:self.mapView action:nil];
  rotate.state = UIGestureRecognizerStateBegan;
  rotate.rotation = MLNRadiansFromDegrees(1);
  ...
  rotate.rotation = MLNRadiansFromDegrees(2);
  [self.mapView handleRotateGesture:rotate];
  ...
  for (NSNumber *degrees in
       @[ @-90, @-10, @10, @10, @30, @90, @180, @240, @460, @500, @590, @800 ]) {
    rotate.state = UIGestureRecognizerStateChanged;
...
NS_INLINE CGFloat MLNScaleFromZoomLevel(double zoom) { return pow(2, zoom); }

__unused NS_INLINE double MLNZoomLevelFromScale(CGFloat scale) { return log2(scale); }
